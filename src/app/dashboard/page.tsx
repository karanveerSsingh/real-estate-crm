...
export default function DashboardHome() {
    ...
    const refreshOverdueCount = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setData((prev: any) =>
            prev ? { ...prev, stats: { ...prev.stats, overdueFollowups: total } } : prev
          );
        }
      } catch {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const hasUrgent = (payload.stats.overdueFollowups > 0 || payload.stats.todayFollowups > 0);
  ...
  const handleOverdueCountChange = (count: number) => {
    setData((prev: any) =>
      prev ? { ...prev, stats: { ...prev.stats, overdueFollowups: count } } : prev
    );
  };
  ...
  if (loading || !data) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-[500px]">
        ...
        <span className="text-sm text-[var(--muted)]">Gathering real-time CRM intelligence...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl relative overflow-hidden group">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-xs font-semibold text-[var(--muted)] uppercase">Total Lead Pool</span>
          ...
          <div className="text-[11px] text-[var(--muted)] mt-3 flex items-center gap-2">
            <span className="text-green-500 font-semibold flex items-center gap-0.5"><UserPlus className="h-3 w-3" />{stats.newLeads}</span> New Leads
            <span className="text-indigo-500 font-semibold">{stats.interestedLeads}</span> Interested
          </div>
        </div>
        ...
        <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl relative overflow-hidden group">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-xs font-semibold text-[var(--muted)] uppercase">Follow-up schedule</span>
              <h3 className="text-2xl font-bold">{stats.todayFollowups} Today</h3>
            </div>
          ...
          <div className="text-[11px] text-[var(--muted)] mt-3 flex items-center gap-2">
            <span className="text-amber-500 font-semibold">{stats.tomorrowFollowups} Tomorrow</span>
            {stats.overdueFollowups > 0 && (
              <button
                type="button"
                onClick={() => setShowOverdueModal(true)}
      ...
      {mounted && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl">
            <h3 className="text-sm font-bold text-[var(--foreground)] mb-4">Monthly Lead Velocity</h3>
            <div className="h-64">
          ...
          <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl">
            <h3 className="text-sm font-bold text-[var(--foreground)] mb-4">Monthly Closed Sales Volume (₹)</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
          ...
          {/* Location Wise Leads */}
          <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl">
            <h3 className="text-sm font-bold text-[var(--foreground)] mb-4">Preferred Location Heatmap</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={graphs.locationWiseLeads} layout="vertical">
                  <XAxis type="number" stroke="var(--muted)" fontSize={10} />
                  <YAxis dataKey="location" type="category" stroke="var(--muted)" fontSize={10} width={80} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)', color: 'var(--foreground)', fontSize: 12 }}
                  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl flex flex-col h-[400px]">
          <div className="flex justify-between items-center pb-3 border-b border-[var(--border)] mb-3">
            ...
            <Link href="/dashboard/calendar" className="text-xs text-blue-500 hover:underline">
              View Calendar
            </Link>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {widgets.pendingFollowups.length === 0 ? (
              ...
              widgets.pendingFollowups.map((task: any) => {
                ...
                const isOverdue = date < new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate());
                
                return (
                  <div key={task._id} className="p-2.5 bg-[var(--background)] border border-[var(--border)] rounded-lg flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[var(--muted)] truncate">
                        Client: {task.customerId?.fullName || 'Unknown'} | {task.customerId?.mobileNumber || ''}
                      </p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${
                          isOverdue ? 'bg-red-500/10 text-red-500 font-bold' : 'bg-slate-500/10 text-[var(--muted)]'
                        }`}>
                          {new Date(task.date).toLocaleDateString()} at {task.time}
                        </span>
                        {isOverdue && <span className="text-[9px] text-red-500 font-semibold animate-pulse">OVERDUE</span>}
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl flex flex-col h-[400px]">
          <div className="flex justify-between items-center pb-3 border-b border-[var(--border)] mb-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--muted)] flex items-center gap-1.5">
              <BellRing className="h-4 w-4 text-blue-500" /> Recent Activity Timeline
            </h4>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pl-2">
            {widgets.recentActivities.length === 0 ? (
              ...
              widgets.recentActivities.map((act: any) => {
                const dateFormatted = new Date(act.timestamp).toLocaleTimeString(undefined, {
                  hour: '2-digit',
                  minute: '2-digit'
                });

                return (
                  <div key={act._id} className="relative pl-4 border-l border-blue-500/20 last:border-0 pb-1.5">
                    <span className="absolute -left-1.5 top-1.5 h-3 w-3 rounded-full bg-blue-500 border-2 border-[var(--card)]" />
                    <div className="text-xs">
                      <span className="font-semibold">{act.customerId?.fullName || 'System'}</span>
                      <span className="text-[10px] text-blue-500 bg-blue-500/10 px-1 py-0.2 rounded ml-2 font-medium">
                        {act.type}
                      </span>
                      <p className="text-[11px] text-[var(--muted)] mt-0.5">{act.description}</p>
                      <span className="text-[9px] text-[var(--muted)] block mt-0.5">{dateFormatted}</span>
                    </div>
                  </div>
                );
              })
        ...
        <div className="p-4 bg-[var(--card)] border border-[var(--border)] rounded-xl flex flex-col h-[400px]">
          <div className="flex justify-between items-center pb-3 border-b border-[var(--border)] mb-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--muted)] flex items-center gap-1.5">
              <FileCheck2 className="h-4 w-4 text-green-500" /> Recently Closed Sales
            </h4>
            <Link href="/dashboard/sold" className="text-xs text-green-500 hover:underline">
              View All
            </Link>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {widgets.recentlySold.length === 0 ? (
              ...
              widgets.recentlySold.map((deal: any) => (
                <div key={deal._id} className="p-2.5 bg-[var(--background)] border border-[var(--border)] rounded-lg flex items-center justify-between gap-2">
      ...
      {showReminderPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={dismissReminder} />
          ...
          <div className="relative w-full max-w-lg bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-2xl p-6 overflow-hidden max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center gap-3 pb-3 border-b border-[var(--border)] mb-4 text-amber-500">
              ...
              <div>
                <h3 className="text-md font-bold text-[var(--foreground)]">Urgent Reminders</h3>
                <p className="text-xs text-[var(--muted)]">You have pending items requiring immediate attention today.</p>
              </div>
            </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-red-500 flex items-center gap-1">
                      <AlertTriangle className="h-3.5 w-3.5" /> Missed / Overdue Follow-ups ({stats.overdueFollowups})
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <div className="space-y-2">
                    {widgets.pendingFollowups
                      ...
                      .slice(0, 3)
                      .map((task: any) => (
                        <div key={task._id} className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl flex justify-between items-center">
                          <div className="min-w-0">
                            <p className="text-xs font-semibold truncate">{task.title}</p>
                            <p className="text-[10px] text-[var(--muted)] mt-0.5">Client: {task.customerId?.fullName || 'Unknown'}</p>
                          </div>
                          <Link 
              ...
              {stats.todayFollowups > 0 && (
                <div className="space-y-2 pt-2 border-t border-[var(--border)]">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-amber-500 flex items-center gap-1">
                    <CalendarDays className="h-3.5 w-3.5" /> Scheduled for Today ({stats.todayFollowups})
                  </span>
                  ...
                  <div className="space-y-2">
                    {widgets.pendingFollowups
                      ...
                      .slice(0, 3)
                      .map((task: any) => (
                        <div key={task._id} className="p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl flex justify-between items-center">
                          <div className="min-w-0">
                            <p className="text-xs font-semibold truncate">{task.title}</p>
                            <p className="text-[10px] text-[var(--muted)] mt-0.5">Time: {task.time} | Client: {task.customerId?.fullName || 'Unknown'}</p>
                          </div>
                          <Link 
                            href={`/dashboard/leads/${task.customerId?._id || task.customerId}`}
                            className="text-[11px] font-semibold text-blue-500 hover:underline shrink-0 pl-2"
