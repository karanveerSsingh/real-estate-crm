...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const customer = await Customer.findOne({ _id: id, userId: session.user.id }).lean();
    ...
    const query: Record<string, unknown> = { userId: session.user.id, status: { $in: ['Available', 'Booked'] }, ...(priceQuery ? { price: priceQuery } : {}) };
